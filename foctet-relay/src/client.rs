...
impl ClientActor {
    async fn run(mut self, cancel: CancellationToken) -> Result<()> {
        if self.dst_node_id.is_zero() {
            tracing::info!("Starting relay control actor loop for NodeId {:?}", self.src_node_id.to_base32());
        } else {
            tracing::info!("Starting relay client actor loop for NodePair (SRC: {:?}, DST: {:?})", self.src_node_id.to_base32(), self.dst_node_id.to_base32());
        }
        loop {
            tokio::select! {
                _ = cancel.cancelled() => {
...
impl ClientManager {
    ...
    // Initial relay handshake. Relay the handshake packet to the destination node.
    pub async fn relay_handshake(&mut self, pair: NodePair) -> Result<()> {
        let dst_node_pair = NodePair::new(pair.dst.clone(), pair.src.clone());
        let frame = if self.connections.contains_key(&dst_node_pair) {
            tracing::info!("Relay Connected Response to the destination node");
            // Relay Connected Response to the destination node
            let frame: Frame = Frame::builder()
                .with_fin(true)
                .with_frame_type(FrameType::Connected)
                .with_operation_id(OperationId(0))
                .as_response()
                .build();
            frame
        }else{
            tracing::info!("Relay Connect Request to the destination node");
            // Relay Connect Request to the destination node
            let frame: Frame = Frame::builder()
                .with_fin(true)
                .with_frame_type(FrameType::Connect)
                .with_operation_id(OperationId(0))
                .with_payload(Payload::handshake(HandshakeData::new(pair.src.clone(), pair.dst.clone(), None)))
                .as_request()
                .build();
            frame
        };
        let packet = Packet::new(pair.src, pair.dst, frame.to_bytes()?);
    ...
    pub async fn register(&mut self, pair: NodePair, stream: NetworkStream, packet_queue_capacity: usize) {
        ...
        tracing::info!("Client connection registered: SRC: {:?}, DST: {:?}", pair.src.to_base32(), pair.dst.to_base32());
        self.connections.insert(pair.clone(), client);
        if !pair.src.is_zero() && !pair.dst.is_zero() {
            match self.relay_handshake(pair).await {
                Ok(_) => {
                    tracing::info!("Relay connection established.");
                },
                Err(e) => {
                    tracing::warn!("Error sending relay handshake: {:?}", e);
                }
            }
        }
    }
