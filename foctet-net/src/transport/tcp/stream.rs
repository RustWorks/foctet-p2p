...
impl FoctetStream for TlsTcpStream {
    ...
    fn split(self) -> (SendStream, RecvStream) {
        ...
        let send_stream = SendStream::Tcp(tcp_send_stream);
        let recv_stream = RecvStream::Tcp(tcp_recv_stream);
        (send_stream, recv_stream)
    }
    fn merge(send_stream: SendStream, recv_stream: RecvStream) -> Result<Self> where Self: Sized {
        match (send_stream, recv_stream) {
            (SendStream::Tcp(tcp_send_stream), RecvStream::Tcp(tcp_recv_stream)) => {
                Ok(Self {
